[中文文档](README.md) | [English Docs](en/README.md) | [English version](en/PRIVACY.md)

# Pesty 隐私说明

最后更新：2026 年 7 月 31 日

Pesty 是一款 macOS 剪贴板历史工具。剪贴板内容默认只保存在本机；项目不提供
剪贴板云服务、账号系统、广告、分析或遥测。只有当用户主动调用翻译或解释，并选择
自己配置的云端 AI 服务时，当前选中的文本才会直接发送给该服务。

## Pesty 保存什么

为了让你重新使用复制过的内容，Pesty 可以保存文本、链接、图片、文件、富文本和
颜色，以及 Pinboard 和相关显示信息。

本地数据默认保存在：

- `~/Library/Application Support/Pesty`
- `~/Library/Preferences/com.bifrostproxy.pesty.plist`

密码管理器标记为 concealed/隐藏的剪贴板内容会被忽略，不会写入历史记录。

## 可选的 iCloud 同步

iCloud 同步默认关闭。启用后，历史记录、Pinboard 和历史记录保留上限会存入你
自己的 iCloud Drive，并通过 Apple 的 iCloud 服务在你的 Mac 之间同步。项目
维护者没有访问这些数据的服务器或账号权限。

## 网络访问

Pesty 不会把剪贴板内容发送给项目维护者或第三方分析服务。应用只在以下产品功能
中访问网络：

- 用户主动使用豆包翻译或解释当前选中的文本。
- 用户主动翻译图片卡片时，Pesty 使用 macOS Vision 在本机临时识别文字；原始图片
  不会发送给翻译服务。若使用豆包，只有识别出的文字会发送。
- 用户主动使用兼容 AI 服务解释当前选中的文本。
- 启动时、每小时或手动检查更新时，读取公开的 GitHub Releases Feed。
- 用户确认安装更新后，从 GitHub Releases 下载发布包。
- 用户点击 GitHub 或“反馈问题”链接后，由系统浏览器打开相应页面。
- 用户主动启用 iCloud 同步后，由 macOS 和 iCloud Drive 处理同步。

云端翻译和解释只发送用户主动处理的当前选中文本或临时 OCR 结果，不会批量上传
剪贴板历史、Pinboard 内容、原始图片或用户文件。OCR 结果不会写入历史或 iCloud。
API Key 保存在 macOS 钥匙串中，不会写入偏好设置或 iCloud。更新请求不包含
剪贴板历史、Pinboard 内容或用户文件内容。

## 辅助功能权限

“直接粘贴到当前应用”会使用 macOS 辅助功能权限，向之前正在使用的应用发送粘贴
快捷键。用户主动触发全局翻译或解释时，Pesty 也会通过辅助功能接口读取当前选中的
文字和选区位置，以便处理文本并在选区附近显示结果。

如果应用没有直接公开选区，Pesty 可能临时发送一次复制快捷键，读取选中文字后完整
恢复原有剪贴板内容；该临时内容不会写入历史。Pesty 不会读取安全输入框中的内容，
也不会使用辅助功能权限持续采集窗口内容、键盘输入或其他应用数据。

## 数据控制

可以在“设置 → 数据”中清除剪贴板历史。卸载应用不会自动删除所有本地或 iCloud
数据；需要时请参考[卸载说明](SUPPORT.md#如何卸载)并在删除前确认数据不再需要。

## 联系方式

隐私问题可以通过
[GitHub Issues](https://github.com/bifrost-proxy/pesty/issues) 提交。请勿在公开
Issue 中附带真实剪贴板内容或其他敏感信息。

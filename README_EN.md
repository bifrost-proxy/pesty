...
| Translation | Translates selected text in any app and text or image cards in Pesty. Image text is first recognized locally with macOS Vision. It supports automatic source-language detection, language swapping, and a configurable target language. Apple Translation is available on macOS 15 and later; Doubao can be configured for macOS 14 or cloud-model use. | Select text in another app and press `⇧⌘T`. In Pesty, select a text or image card and press `⇧⌘T` or choose “Translate” from its context menu. See the [Translation Guide](docs/en/TRANSLATION.md). |
...
| Privacy and security | History stays local by default, and Pesty can ignore content marked as concealed by password managers. Image OCR runs locally; cloud translation and explanation send only text or OCR output the user explicitly processes, never the original image. API keys are stored in macOS Keychain. | Enable “Ignore concealed content” under “Settings → General.” iCloud and cloud AI services remain opt-in. |
...
| `⇧⌘T` | Globally translate selected text, or translate the selected Pesty text or image card while the panel is open; customizable in Settings |
